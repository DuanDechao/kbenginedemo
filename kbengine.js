...
KBEngine.Bundle = function()
{
	...
	this.msgid = 0;
	//---------------------------------------------------------------------------------
	this.newMessage = function(msgid)
	{
		this.fini(false);
		...
		//}
		this.writeUint32(this.msgid);
		this.messageLengthBuffer = new Uint8Array(this.stream.buffer, this.stream.wpos, 4);
		this.writeUint32(0);
		this.messageLength = 0;
	...
	this.writeMsgLength = function(v)
	{
		...
		this.messageLengthBuffer[2] = v >> 16 & 0xff;
		this.messageLengthBuffer[3] = v >> 24 & 0xff;
	...
	this.fini = function(issend)
	{
		...
		{
			this.writeMsgLength(this.messageLength + 8);
			if(this.stream)
...
KBEngine.Message = function(id, name, length, argstype, args, handler)
{
	...
	KBEngine.ERROR_MSG("KBEngine.Message:: NEW Message handler(" + this.handler + ")"); 
...
KBEngine.KBEngineApp = function(kbengineArgs)
{
	...
	{ 
		...
		while(stream.rpos < stream.wpos)
		{
			...
			else
			{
				...
				msglen = stream.readUint32();
				var wpos = stream.wpos;
				var rpos = stream.rpos + msglen - 8;
				KBEngine.INFO_MSG("dddddddddddddddddddddddddddddddddddddonmessage[" + msglen + "] msg(" + msgid + ")!" + stream.rpos + "," + stream.wpos);
				stream.wpos = rpos;
				msgHandler.handleMessage(stream);
				stream.wpos = wpos;
				stream.rpos = rpos;
	...
	*/
	...
	{  
		...
		{
			var bundle = new KBEngine.Bundle();
			//KBEngine.messages.Loginapp_login.id = 1;
			bundle.newMessage(1);
			...
			bundle.writeString(KBEngine.app.password);
			bundle.send(KBEngine.app);
			KBEngine.INFO_MSG("KBEngineApp::login_loginapp: send data.............................successfully!"); 
		}
	}
	...
	this.Client_onCreatedProxies = function(stream)
	{
		//KBEngine.app.entity_uuid = stream.readUint64();
	...
	KBEngine.clientmessages[10] = new KBEngine.Message(10, "Client_onCreatedProxies", 0, 0, new Array(), KBEngine.app["Client_onCreatedProxies"]);
